...
```{r plot, include = TRUE}
base5b + geom_segment( aes(x=reorder(crime, pctCol), xend=reorder(crime, pctCol), y=0, yend=pctCol)) +
  facet_grid( ~ daytime) +
  geom_point( size=.04, color="red", fill=alpha("orange", 0.3), alpha=0.7, shape=21, stroke=2) +
  geom_text(aes(label=paste0(round(pctCol*100,2),"%")), position=position_dodge(width=0.9), vjust=-0.50, hjust = -.050, size = 3) +
  coord_flip() +
  theme(axis.text.y = element_text(size=5,angle = 45)) 
```
